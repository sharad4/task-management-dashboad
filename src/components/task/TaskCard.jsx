...
const TaskCard = memo(({ task, onUpdate, onDelete, isSelected, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3 mx-5">
        <h3 className="font-semibold text-lg pr-8">{task.title}</h3>
        <div className="flex gap-2">
          <button
            onClick={() => onUpdate(task)}
            className="p-2 text-gray-500 hover:text-blue-50 hover:bg-blue-500 rounded-full transition-colors"
